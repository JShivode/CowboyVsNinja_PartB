...
void Team2::attack(Team* enemyTeam){
    ...
    if (!getLeader()->isAlive()) { //if leader of attacking team is dead, replace with closest to the dead leader.
        setLeader(chooseNewLeader());
        if(!getLeader() || !getLeader()->isAlive())
...
void Team2::print() {
    ...
    cout<<"Team2, leader: #1("<<getLeader()->print()<<") "<<endl;
     
    k++;
    for (Character* member : Team2::getTeamMembers()) {
            if(member != getLeader()){
